...
import { createAssetType, updateAssetType } from '@/lib/actions/asset-type-action'
import { assetTypeDefaultValues } from '@/lib/constants'
import { assetTypeSchema } from '@/lib/validators'
...
const AssetTypeForm = ({ data, update = false }: { data?: AssetType, update: boolean }) => {
    const router = useRouter()
    const id = data?.id;

    const form = useForm<z.infer<typeof assetTypeSchema>>({
        resolver: zodResolver(assetTypeSchema),
        defaultValues: data || assetTypeDefaultValues
    })
            ...
            <form className='space-y-4' onSubmit={form.handleSubmit(onSubmit, (errors) => console.log(errors))}>
                <div className='grid grid-cols-2 gap-2'>
                    <div className='flex flex-col gap-5'>
                        <FormField
                            ...
                            name='name'
                            render={({ field }: {
                                field: ControllerRenderProps<z.infer<typeof assetTypeSchema>, "name">
                            }) => (
                                <FormItem>
                    ...
                    <div className='flex flex-col gap-5'>
                        <FormField
                            ...
                            name='status'
                            render={({ field }: {
                                field: ControllerRenderProps<z.infer<typeof assetTypeSchema>, "status">
                            }) => (
                                <FormItem>
                ...
                <div className='flex flex-col gap-5'>
                    <FormField
                        ...
                        name='description'
                        render={({ field }: {
                            field: ControllerRenderProps<z.infer<typeof assetTypeSchema>, "description">
                        }) => (
                            <FormItem>
